...
const FavoriteMovieCard = ({movie}) => {
  const { removeMovieToFavorites } = useContext(MovieContext);
  return (
    <div>
      <div className="movie">
          ...
          <div className="result-image">
            ...
            <div className="result-overlay absolute inset-0 flex flex-col items-center justify-center opacity-0 hover:opacity-100 transition-opacity duration-300 ease-in-out bg-white">
              ...
              <div className="result-actions mt-2">
                <button onClick={() => removeMovieToFavorites(movie.id)} className="border-2 border-orange bg-navyBlue text-white hover:bg-orange hover:border-navyBlue hover:text-navyBlue px-4 py-2">
                  Remove
                </button>
              </div>
            </div>
